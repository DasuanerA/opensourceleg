...
SERVO_PARAMS: dict[str, Any] = {
    ...
    "AK60-6": {
        "P_min": -32000,  # -3200 deg
        "P_max": 32000,  # 3200 deg
        "V_min": -30000,  # ERPM (adjusted for different pole pairs)
        "V_max": 30000,  # ERPM (adjusted for different pole pairs)
        "Curr_min": -13100,  # -13.1A (protocol units)
        "Curr_max": 13100,  # 13.1A (protocol units)
        "Kt_actual": 0.135,  # Nm/A (from datasheet)
        "GEAR_RATIO": 6.0,  # 6:1 reduction ratio
        "NUM_POLE_PAIRS": 14,  # pole pairs from datasheet
    },
    "CAN_PACKET_ID": {
...
class TMotorServoActuator(ActuatorBase):
    ...
    Example:
        >>> with TMotorServoActuator(motor_type="AK60-6", motor_id=1) as motor:
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        Args:
            ...
            motor_type: motor model (AK80-9, AK10-9, AK60-6)
            motor_id: CAN ID
...
if __name__ == "__main__":
    ...
    try:
        actuator = TMotorServoActuator(motor_type="AK60-6", motor_id=1, offline=True)
